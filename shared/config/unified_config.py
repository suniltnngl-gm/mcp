...
class UnifiedConfig:
    """Smart auto-configuration and environment management for AI Orchestra."""
    ...
    def __init__(self):
        ...
        self.env_info_file = Path(os.path.dirname(os.path.abspath(__file__))) / 'environment_info.json'

    def read_env_info(self):
        """Reads the environment information from the JSON file."""
        if not self.env_info_file.exists():
            return {}
    ...
    def write_env_info(self, data):
        """Writes the environment information to the JSON file."""
        os.makedirs(self.env_info_file.parent, exist_ok=True)
        with open(self.env_info_file, 'w') as f:
            json.dump(data, f, indent=2)

    def update_env_info(self, key, value):
        """Updates a specific key-value pair in the environment information."""
        data = self.read_env_info()
        data[key] = value
        self.write_env_info(data)
    ...
    def generate_smart_provider_priority(
        self,
        keys_by_provider: dict[str, list[KeyInfo]],
    ) -> str:
    ...
    def create_shell_exports(
        ...
        for provider in sorted(free_providers):
            ...
            exports.append(f"# {provider.title()} configuration")
            ...
            if provider in keys_by_provider:
                ...
                for key_info in keys:
                    exports.append(f'{key_info.name}="{key_info.value}"')
                ...
                for i in range(
                    ...
                    exports.append(f'# {base_key}{suffix}=""')
            else:
                # Provider not configured, add commented placeholders
                exports.append(f'# {base_key}=""')
                exports.append(f'# {base_key}_2=""')
        ...
        # Git configuration
        exports.append("# 📦 Git/GitHub Configuration")
        exports.append("# ========================")

        if self.git_status.git_author_name:
            exports.append(f'GIT_AUTHOR_NAME="{self.git_status.git_author_name}"')
        else:
            exports.append('# GIT_AUTHOR_NAME=""')

        if self.git_status.git_author_email:
            exports.append(f'GIT_AUTHOR_EMAIL="{self.git_status.git_author_email}"')
        else:
            exports.append('# GIT_AUTHOR_EMAIL=""')
        ...
        if self.git_status.gh_token_set:
            ...
            exports.append('# GH_TOKEN=""  # GitHub CLI token for API access')
    ...
    def create_env_file(
        ...
        env_content = [
            ...
            'AI_STRATEGY="cost_optimized"  # Using free providers first',
            'AI_DEBUG="false"',
            'AI_TIMEOUT="30"',
            "",
        ]
    ...
    def print_recommendations(self, keys_by_provider: dict[str, list[KeyInfo]]):
        ...
        single_key_providers = [p for p in keys_by_provider if len(keys_by_provider[p]) == 1]
    ...
    def run_auto_config(
        ...
        if generate_env:
            ...
            if RICH_AVAILABLE:
                ...
                console.print(
                    "   Copy this to .env to use these settings."
                )
            else:
