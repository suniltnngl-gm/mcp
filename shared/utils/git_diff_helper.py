...
class GitDiffHelper:
    ...
    def get_file_content_at_hash(self, file_path: Path, git_hash: str) -> Tuple[bool, str, str]:
        """Get the content of a file at a specific git hash.
        
        Args:
            file_path: Path to the file (relative to repo_root)
            git_hash: The git hash (or revision) to get the content from
        ...
        try:
            relative_path = file_path.relative_to(self.repo_root)
            cmd = ["git", "show", f"{git_hash}:{relative_path}"]
            ...
            return False, "", f"Failed to get file content at hash: {e.stderr}"
        except Exception as e:
            return False, "", f"Error getting file content at hash: {str(e)}"

    def get_current_file_git_hash(self, file_path: Path) -> Tuple[bool, str, str]:
        """Get the current git hash of a file.
        
        Args:
            file_path: Path to the file (relative to repo_root)
            
        Returns:
            Tuple of (success, git_hash, error_message)
        """
        if not self._is_git_repo:
            return False, "", "Not a git repository"
        ...
        try:
            relative_path = file_path.relative_to(self.repo_root)
            cmd = ["git", "hash-object", str(relative_path)]
            ...
            return False, "", f"Failed to get current file hash: {e.stderr}"
        except Exception as e:
            return False, "", f"Error getting current file hash: {str(e)}"
