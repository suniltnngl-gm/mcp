"""Upload the contents of your Downloads folder to Dropbox (API v2)."""
...
parser.add_argument('--token', default=TOKEN,
                    help='Access token (or set DROPBOX_ACCESS_TOKEN env var)')
...
def main():
    ...
    if sum([bool(b) for b in (args.yes, args.no, args.default)]) > 1:
        ...
        sys.exit(2)
    if not args.token:
        print('--token is mandatory (or set DROPBOX_ACCESS_TOKEN env var)')
        sys.exit(2)
